...
public class Animal extends Actor {
	...
	public void act(long now) {
		...
		if (getIntersectingObjects(Log.class).size() >= 1 && !noMove) {
		...
		else if (getIntersectingObjects(Secret.class).size() >= 1) {
			getIntersectingObjects(Secret.class);
			if (getIntersectingObjects(Secret.class).get(0).isActivated()) {
				end--;
				points=50000;
			}
			points=50000;
			changeScore = true;
			w=800;
			getIntersectingObjects(Secret.class).get(0).setEnd();
			end=6;
			setX(300);
			setY(679.8+movement);
		}
